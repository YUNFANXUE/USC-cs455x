...
public class VisibleField {
   ...
   */
   ...
   private int numMineHasGuessed;          // number of mines to guess
   ...
   */     
   ...
   public int numMinesLeft() {
      return mineFieldVisible.numMines() - numMineHasGuessed;
   }
   ...
   /**
      ...
      @return false iff you uncover a mine at (row, col), else return true
      PRE: getMineField().inRange(row, col)
    */
   ...
   public boolean isGameOver() {
      if (gameOver) {
         ...
         if (isAllNotMineUncover()) {
            ...
            for (int i = 0; i < numVFARows; i++) {
               for (int j = 0; j < numVFACols; j++) {
                  if(mineFieldVisible.hasMine(i, j)) {
                     visibleFieldArray[i][j] = MINE_GUESS;
                  }
               }
            }
         }
   ...
   /**
      ...
      @param row of the square
      @param col of the square
      @return true if the square is uncovered, else return false
      PRE: getMineField().inRange(row, col)
    */
   ...
   */
   private void exploration(int row, int col) {
      ...
      //if it is already detected, or has marked as MINE_GUESS, we return
      if (visibleFieldArray[row][col] == MINE_GUESS || visibleFieldArray[row][col] >= 0) {
         return;
      } 
